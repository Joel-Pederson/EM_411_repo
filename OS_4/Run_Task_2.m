...
designs{1}.archTypes{1} = 'road';
designs{1}.fleetSizes(1) = 25; % Our assumption for fleet size
...
% designs{5}.fleetSizes.road = 15; % Assumption: 15 road vehicles
% designs{5}.fleetSizes.bike = 50; % Assumption: 50 bikes

%% -- Execute Model -- %%
for ii = 1:length(designs)
    design = designs{1};

    if strcmp(design.archTypes, 'road')
    ...
    elseif strcmp(design.archTypes, 'bike')
        [Bike_EV_Design, cost, isValid] = calculateBikeVehicle(design, bikeDB);
        if ~isValid
            continue
        end

    elseif strcmp(design.archTypes, 'mixed')
